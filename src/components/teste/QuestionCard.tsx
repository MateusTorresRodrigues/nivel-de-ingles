...
export function QuestionCard({ questao, locked, onSelect }: QuestionCardProps) {
  return (
    <div
      key={questao.id}
      className="animate-q-in bg-white border border-fg/10 rounded-[20px] py-[26px] px-[22px] shadow-[0_1px_2px_rgba(28,27,24,.04)]"
    >
        ...
        {questao.options.map((option, i) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex items-center gap-3 w-full text-left cursor-pointer text-[15.5px] leading-[1.4] -tracking-[0.01em] py-3.5 px-[15px] rounded-xl transition-[border-color,background,transform] duration-[160ms]",
                selected ? "border-[1.5px] border-accent bg-accent-soft-2 text-fg scale-[0.99]" : "border-[1.5px] border-fg/12 bg-white text-fg hover:border-fg/34",
              )}
            >
              <span
                className={cn(
                  "flex-none w-6 h-6 rounded-[7px] flex items-center justify-center text-[11.5px] font-semibold",
